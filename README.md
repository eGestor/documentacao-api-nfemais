...
O [NFeMais](https://v4.nfemais.com.br) disponibiliza uma API RESTful que permite o acesso aos módulos do sistema.
...
## URLs de acesso
O NFe+ não possui sandbox (ambiente de homologação). Cada conta do sistema é isolada das outras (multi-tenant), sugerimos aos desenvolvedores que criem uma conta de testes, e depois utilizem a conta de produção com os dados dos clientes.
...
URL homologação/produção (access_token): https://api.nfemais.com.br/api/oauth/access_token

URL homologação/produção (ex: contatos): https://api.nfemais.com.br/api/v1/contatos
...
Para testar a API, crie uma conta no NFe+ e entre no sistema utilizando as credenciais informadas.  
Para gerar o token da API (personal_token)

### Obtendo o personal_token

Para testar a API do NFe+ (sandbox), sugerimos a criação de uma conta de testes.
...
> POST https://api.nfemais.com.br/api/oauth/access_token

O `personal_token` é do formato JWT e contém informações da empresa (subdomínio) e do usuário.
...
### Listar (List) [GET /produtos{?filtro,codCategoria,arquivado,anotInternas,Norig,INCM,codConfigTrib,IcProd,updatedBefore,updatedAfter,fields,orderBy}]
...
+ Parameters
    ...
    + orderBy (optional) - Permite definir a ordenação da listagem. Informe o campo e a forma de ordenação (ascendente ou descendente) separados por vírgula. Só é possível definir uma ordenação por requisição. Valores possíveis:
        * codigo, codCategoria, descricao, codigoProprio, precoVenda, origemFiscal, unidadeTributada, refEanGtin, ncm, excecaoIPI, codigoCEST, pesoBruto, pesoLiquido, anotacoesNFE, anotacoesInternas, tags, dtCad
...
### Resumo de notas [POST /relatorios/nfeResumo]
            ...
            [
                {
                    ...
                    "usuario": "admin",
                    "msg": "Salvou nota",
                    "obs": "ambiente: Homologação; numNota: 1; modelo: 55; serie: 1; chave: {{Chave da nota}}; ",
                    "data": "2025-01-20 14:56:53"
                },
                {
                    "codigo": "23",
                    "codModulo": "1",
...
+ Parameters
    ...
    + fields (string, optional) - Permite definir quais os campos serão retornados pela api. Informe separado por vírgula. Valores possíveis:
        * codigo, numero, cliente, chave, email, ambiente, serie, modelo, dtEmissao, cfop, valor, situacao
...
# Webhooks [/webhooks]
O NFeMais conta com uma estrutura de webhooks. Através dela, será disparada uma requisição para determinada URL do seu sistema sempre que algum registro for criado, editado ou excluído. 
...
Sempre que um produto, cliente ou usuário for criado, editado ou excluído, você receberá um webhook no endpoint cadastrado, no seguinte formato (form-data):
...
+ date: Data que a alteração ocorreu no NFeMais
+ module: Módulo do registro alterado. Valores possíveis: "produtos", "contatos" ou "usuarios"
+ securityToken: Sua chave de segurança

*ATENÇÃO: O timeout para resposta ao POST do webhook é de 3 segundos. Serão feitas 5 tentativas em caso de falha. Sugerimos a utilização de queues para processamento dos webhooks enviados pelo NFeMais, a fim de evitar timeouts.*

No momento do cadastro do webhook, é necessário definir quais os módulos serão ativados através dos parâmetros "produtos", "contatos" e "usuarios".
...
+ Response 200 (application/json)
    ...
    + Body
            ...
            {
              "endpoint": "http://www.example.com/webhookNFeMais",
              "securityToken": "01a161daaaf265968c1f36a19ef8f4",
              "produtos": true,
              "contatos": false,
              "usuarios": true,
